...
function useCard(card) {
    ...
    if (card.isChampion()) {
        if (cycle.championInDeck !== card) {
            return;
        }

        cycle.limbo = card;
        let index = cycle.activehand.removeCard(card);
        cycle.activehand.insert(index, cycle.drawpile.popCardAtStart());
    } else {
...
function undo() {
    ...
    cycle.limbo = lastAction[3];
    cycle.championInDeck = lastAction[4];
    cycle.cardsPlayed = lastAction[5];
...
function updateStyles() {
    ...
    let limboElement = document.getElementById("limbo");
    let limboTextElement = document.getElementById("cycle-limbo");
    if (cycle.limbo === null) {
        limboTextElement.style.visibility = "hidden";
        limboElement.src = "MysteryCard.png";
        limboElement.style.visibility = "hidden";
    } else {
        limboTextElement.style.visibility = "visible";
        limboElement.src = cycle.limbo.getIcon();
        limboElement.style.visibility = "visible";
    }
...
function useCardFromDrawPile(card) {
    ...
    if (card.isChampion()) {
        if (!cycle.championInDeck.equals(card)) {
            return;
        }
...
document.getElementById("limbo").addEventListener("click", () => {
    if (killChampion()) {
        saveState();
        updateStyles();
    }
